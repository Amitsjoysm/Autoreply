...
async def create_automated_followups(email: Email, time_reference: dict, ai_service: AIAgentService):
    """Create automated follow-ups for time-based requests"""
    try:
        ...
        # Calculate follow-up dates: 2, 4, 6 days after target date
        follow_up_intervals = [2, 4, 6]
        follow_ups_created = []
        
        for interval in follow_up_intervals:
            follow_up_date = target_date + timedelta(days=interval)
            ...
            follow_ups_created.append({
                "scheduled_at": follow_up_date.isoformat(),
                "days_after_target": interval
            })
            
            logger.info(f"Created automated follow-up for email {email.id} at {follow_up_date.isoformat()}")
